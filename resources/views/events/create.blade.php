    ...
    <form method="POST" action="/events" enctype="multipart/form-data">
      ...
      <div class="mb-3">
        ...
        <input type="text" class="border border-slate-200 rounded-lg px-4 py-2 w-full font-light" name="name"
          value="{{old('name')}}" placeholder="Ginfest 2022" />
      ...
      <div class="mb-3">
        ...
        <input type="datetime-local" name="time" class="border border-slate-200 rounded-lg px-4 py-2 w-full font-light">
      ...
      {{-- Artists --}}
      <div class="mb-3">
        <label for="artist" class="inline-block text mb-1 text-slate-600">Preforming artists</label>
        <select name="artist" class="border border-slate-200 rounded-lg px-4 py-2 w-full font-light">
          @foreach ($artists as $artist)
          <option value="{{ $artist->id }}" @selected(old('artist')==$artist)>
            {{ $artist->name }}
          </option>
          @endforeach
        </select>
